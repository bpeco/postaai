...
> **Auth: el cron necesita `ANTHROPIC_API_KEY`, NO el OAuth de Claude Max.** Esta es la causa del apagón del cron del 16 al 18/06: el [15/06/2026 Anthropic cambió la política](https://www.morphllm.com/claude-code-usage-limits) — el uso **no-interactivo** (`claude -p`, GitHub Actions, Agent SDK) en planes de subscription pasó a salir de un **crédito mensual separado** ($100/mes en Max 5x). Al agotarse ese crédito, los requests **grandes** (como la fase 7) se cuelgan indefinidamente (respuesta nunca streamea, 0 bytes, timeout); los chicos pasan. Local anda porque es uso *interactivo* (otro pool). El fix es cargar `ANTHROPIC_API_KEY` (Console, pay-per-use): el script desactiva el OAuth si esa var está presente. **No era la versión del CLI** (pinear una vieja igual cuelga) **ni los tools.**
>
> Higiene del helper `claude_cap` (en `run-digest.sh`), que se queda igual: (a) `--tools ""` — el transform es puro JSON→JSON, no necesita tools; (b) `claude_cap` envuelve cada llamada con timeout (`CLAUDE_TIMEOUT`=420s vía `timeout`/`gtimeout`) → un cuelgue corta y reintenta y el run **falla ruidoso** en vez de morir en silencio a los 15min. (`--bare` NO: suprime el output de `-p`.)
...
- `scripts/run-digest.sh` — orchestrator. PATH explícito al tope (corre igual bajo launchd que interactivo). Helper `claude_cap` envuelve TODA llamada a claude con `--tools ""` + timeout (`CLAUDE_TIMEOUT`=420s default). Fase 7 exige `>= MIN_CARDS` (3) o reintenta/falla — un `{"cards":[]}` no pasa. La fase 7 (cards) corre en `CARDS_MODEL` (default del script `claude-opus-4-8`, conocido-bueno: ~3.5min) + `CARDS_EFFORT` (default vacío). **El cron en la nube ([.github/workflows/digest.yml](../.github/workflows/digest.yml)) pisa esto a `CARDS_MODEL=claude-sonnet-4-6 CARDS_EFFORT=low` + `CLAUDE_TIMEOUT=600`.**
  - **Causa raíz del FAILED del 19/06 (medido local, input real):** NO era Sonnet, era **el effort**. Sonnet 4.6 **sin effort / medium** dispara ~20 min de thinking (1197s, output truncado) → choca el cap de 420s → 2 intentos parciales = los ~46K tok del CSV + FAILED. Con **`--effort low`: 232s, 12 cards, JSON válido, voz rioplatense OK, sin Spanglish, y tags de entidad correctos** (GLM-5.2→`Z.ai`, donde Haiku ponía "DeepSeek"). Por eso la nube usa Sonnet+low y subimos el cap a 600s de margen.
  - Antes de cambiar modelo/effort, **medir local y gratis** (cae en Max OAuth, no toca la API key): correr `claude -p cards.md` directo sobre un `/tmp/digest-cards-input-STAMP.json` ya generado (input fijo = comparación justa, un solo intento sin loop ni cap): `cat /tmp/digest-cards-input-STAMP.json | claude --tools "" --model claude-sonnet-4-6 --effort low -p "$(cat prompts/cards.md)"`. Las fases 4-6 NO usan esto — corren local con la subscription.
